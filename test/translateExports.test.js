...
test.failing('translate no absolute file fail', async t =>{
    let results = await translate(['./feature/testWrite1.md'])
    t.fail()
})
...
test.serial('translate absolute file ', async t =>{
	let results = await translate({aFile:__dirname+'/feature/testWrite1.md', api:'google'})
	results = results.map(x =>x.text)
    t.is(results.length, 0)
})

test.serial('translate absolute file from zh to en', async t =>{
	let results = await translate({'aFile':__dirname+'/feature/testWrite1.md',tF:'en',tT:'zh'})
	results = results.map(x =>x.text)
    t.is(results.length, 0)
})
